...
也可以直接双击 `index.html` 打开。当前版本默认使用本地 HTML，不再依赖 `127.0.0.1:8765` 服务。
...
- `keep-ielts-server.ps1`：旧版本地服务脚本，当前默认不再使用。
- `启动雅思训练器.bat`：一键启动脚本。
- `启动雅思训练器.ps1`：直接打开本地 `index.html`。
- `停止雅思训练器.bat` / `停止雅思训练器.ps1`：停止旧版本地服务。
